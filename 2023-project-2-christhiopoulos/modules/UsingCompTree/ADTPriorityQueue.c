...
// Επιστρέφει την τιμή του κόμβου node_id
 ...
 Pointer node_value(PriorityQueue pqueue, int node_id) {
	// τα node_ids είναι 1-based, το node_id αποθηκεύεται στη θέση node_id - 1
       CompTree tree=comp_tree_get_subtree(pqueue->tree,node_id);
	   return comp_tree_value(tree);
}
...
static void node_swap(PriorityQueue pqueue, int node_id1, int node_id2) {
	CompTree tree1=comp_tree_get_subtree(pqueue->tree,node_id1);
	CompTree tree2=comp_tree_get_subtree(pqueue->tree,node_id2);
	Pointer value1 = node_value(pqueue, node_id1);
	Pointer value2 = node_value(pqueue, node_id2);
	CompTree new_tree2=comp_tree_create(value1,comp_tree_left(tree2),comp_tree_right(tree2));
	CompTree new_tree1=comp_tree_create(value2,comp_tree_left(tree1),comp_tree_right(tree1));     
	CompTree tree3=comp_tree_replace_subtree(pqueue->tree,node_id1,new_tree1);
	pqueue->tree=tree3;
	CompTree tree4=comp_tree_replace_subtree(pqueue->tree,node_id2,new_tree2);  
    pqueue->tree=tree4;
}
...
static void naive_heapify(PriorityQueue pqueue, Vector values) {	
	int n = vector_size(values);
	for(int i=0;i<n;i++){	
		CompTree tree=comptree_insert_last(pqueue->tree,vector_get_at(values,i));
    	pqueue->tree=tree;
	} 
    for (int i = n/2 ;i >=0; i--) {
...
PriorityQueue pqueue_create(CompareFunc compare, DestroyFunc destroy_value, Vector values) {
	...
	pqueue->tree = comp_tree_create(NULL,NULL, NULL);
...
int pqueue_size(PriorityQueue pqueue) {
	...
	return comp_tree_size(pqueue->tree);
}

Pointer pqueue_max(PriorityQueue pqueue) {
	CompTree tree= comp_tree_get_subtree(pqueue->tree, 0);	
	return comp_tree_value(tree);
}

void pqueue_insert(PriorityQueue pqueue, Pointer value) {
	
	CompTree tree=comptree_insert_last(pqueue->tree,value);
	pqueue->tree=tree;   
...
void pqueue_remove_max(PriorityQueue pqueue) {
	...
	CompTree tree=comptree_remove_last(pqueue->tree);
	pqueue->tree=tree;
...
static void tree_destroy(CompTree tree,PriorityQueue pqueue){
	if(tree==NULL){
		return;
	}	
	if(tree!=NULL){
		tree_destroy(comp_tree_left(tree),pqueue);
		tree_destroy(comp_tree_right(tree),pqueue);
		if(pqueue->destroy_value!=NULL){
			pqueue->destroy_value(comp_tree_value(tree));
		}        
...
void pqueue_destroy(PriorityQueue pqueue) {
    ...
    tree_destroy(pqueue->tree,pqueue);
	free(pqueue);
}
